...
/***********************************************************************//**
* @brief GCTAModelSkyCube class extension
***************************************************************************/
%extend GCTAModelSkyCube {
    GCTAModelSkyCube copy() {
        return (*self);
    }
%pythoncode {
    def __getstate__(self):
        state = (self.filename(), self.spectral(), self.temporal(),
                 gammalib.GModelData.__getstate__(self))
        return state
    def __setstate__(self, state):
        if state[0] == '' and state[1] == None and state[2] == None:
            self.__init__()
        elif state[2] == None:
