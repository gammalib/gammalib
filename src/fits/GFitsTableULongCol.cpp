...
GFitsTableULongCol::GFitsTableULongCol(const std::string& name,
                                       const int&         nrows,
                                       const int&         size) :
                    GFitsTableCol(name, nrows, size, 4)
{
    // Initialise class members for clean destruction
    init_members();
 ...
 * loaded into memory.
 ***************************************************************************/
void GFitsTableULongCol::insert(const int& row, const int& nrows)
{
 ...
 ***************************************************************************/
void GFitsTableULongCol::remove(const int& row, const int& nrows)
{
    // Make sure that row is valid
    if (row < 0 || row >= m_length) {
 ...
 ***************************************************************************/
void GFitsTableULongCol::resize_data(const int& index, const int& number)
{
    ...
    if (number != 0) {
        ...
        if (number < 0) {
            ...
            unsigned long* dst  = m_data + index;
            unsigned long* src  = m_data + left;
            int            num  = m_size - left;
            for (int i = 0; i < num; ++i) {
                *dst++ = *src++;
            }
