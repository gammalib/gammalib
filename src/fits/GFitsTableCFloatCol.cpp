...
#define G_INSERT                    "GFitsTableCFloatCol::insert(int&, int&)"
#define G_REMOVE                    "GFitsTableCFloatCol::remove(int&, int&)"
                                                      ...
                                                      const int& inx) const
{
    // If data are not available then load them now
    if (m_data == NULL) {
        const_cast<GFitsTableCFloatCol*>(this)->fetch_data();
    }
 ...
 ***************************************************************************/
double GFitsTableCFloatCol::real(const int& row, const int& inx) const
{
    // If data are not available then load them now
    if (m_data == NULL) fetch_data();
 ...
 ***************************************************************************/
int GFitsTableCFloatCol::integer(const int& row, const int& inx) const
{
    // If data are not available then load them now
    if (m_data == NULL) fetch_data();
 ...
 ***************************************************************************/
void GFitsTableCFloatCol::insert(const int& rownum, const int& nrows)
{
 ...
 * will be loaded into memory.
 ***************************************************************************/
void GFitsTableCFloatCol::remove(const int& rownum, const int& nrows)
{
    // Make sure that rownum is valid
    if (rownum < 0 || rownum >= m_length) {
...
void GFitsTableCFloatCol::copy_members(const GFitsTableCFloatCol& column)
{
    ...
    if (not_loaded) {
        const_cast<GFitsTableCFloatCol*>(&column)->fetch_data();
    }
    ...
    if (not_loaded) {
        const_cast<GFitsTableCFloatCol*>(&column)->release_data();
    }
    #endif
 ...
 ***************************************************************************/
void GFitsTableCFloatCol::fetch_data(void) const
{
    // Save column (circumvent const correctness)
    const_cast<GFitsTableCFloatCol*>(this)->load_column();
 ...
 * copy constructor.
 ***************************************************************************/
void GFitsTableCFloatCol::copy_data(const GFitsTableCol& column)
{
    // Type-cast to the correct column type
    const GFitsTableCFloatCol* ptr = static_cast<const GFitsTableCFloatCol*>(&column);
