 ...
 ***************************************************************************/
/**
 * @file GFitsTableFloatCol.cpp
 * @brief FITS table single precision column class implementation
...
#define G_INSERT                     "GFitsTableFloatCol::insert(int&, int&)"
#define G_REMOVE                     "GFitsTableFloatCol::remove(int&, int&)"
...
const float& GFitsTableFloatCol::operator() (const int& row, const int& inx) const
{
    // If data are not available then load them now
    if (m_data == NULL) {
        const_cast<GFitsTableFloatCol*>(this)->fetch_data();
    }
 ...
 ***************************************************************************/
void GFitsTableFloatCol::clear(void)
{
 ...
 ***************************************************************************/
void GFitsTableFloatCol::insert(const int& rownum, const int& nrows)
{
 ...
 * will be loaded into memory.
 ***************************************************************************/
void GFitsTableFloatCol::remove(const int& rownum, const int& nrows)
{
    // Make sure that rownum is valid
    if (rownum < 0 || rownum >= m_length) {
 ...
 ***************************************************************************/
void GFitsTableFloatCol::fetch_data(void) const
{
    // Save column (circumvent const correctness)
    const_cast<GFitsTableFloatCol*>(this)->load_column();
 ...
 * copy constructor.
 ***************************************************************************/
void GFitsTableFloatCol::copy_data(const GFitsTableCol& column)
{
    // Type-cast to the correct column type
    const GFitsTableFloatCol* ptr = static_cast<const GFitsTableFloatCol*>(&column);
