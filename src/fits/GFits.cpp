 ...
 *
 * @todo In principle, this method should also delete the GFitsHDU elements
 *       in the container, but doing so leads to severe segmentation
 *       faults. We have to check how we should properly handle this.
 ***************************************************************************/
void GFits::free_members(void)
{
    ...
    for (int i = 0; i < m_hdu.size(); ++i) {
        if (m_hdu[i] != NULL) {
            m_hdu[i]->m_fitsfile = NULL; // We need this since FITS file is now closed
            delete m_hdu[i];
            m_hdu[i] = NULL;
        }
    }
