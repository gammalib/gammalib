/***************************************************************************
 *        GFitsImageLongLong.i - Long long integer FITS image class        *
 * ----------------------------------------------------------------------- *
...
%typemap(in) const long long* pixels {
    ...
    if (num > 0) {
        ...
        for (int i = 0; i < num; ++i) {
            ...
            $1[i] = (long long)PyInt_AsLong(s);
        }
    }
}
%typemap(typecheck, precedence=SWIG_TYPECHECK_INTEGER) const long long* pixels {
    $1 = PyList_Check($input) ? 1 : 0;
}
%typemap(freearg) const long long* pixels {
    if ($1) {
        free($1);
    }
}
...
%extend GFitsImageLongLong {
    long long __getitem__(int GTuple1D4D[]) {
    ...
    void __setitem__(int GTuple1D4D[], long long value) {
        if (GTuple1D4D[0] == 1) {
