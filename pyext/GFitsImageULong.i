...
%typemap(in) const unsigned long* pixels {
    if (!PyList_Check($input)) {
        SWIG_exception(SWIG_ValueError, "List expected");
    }
    Py_ssize_t num = PyList_Size($input);
    if (num > 0) {
        $1 = (unsigned long*)malloc(num*sizeof(unsigned long));
        for (int i = 0; i < num; ++i) {
            ...
            $1[i] = (unsigned long)PyInt_AsLong(s);
        }
    }
}
%typemap(typecheck, precedence=SWIG_TYPECHECK_INTEGER) const unsigned long* pixels {
    $1 = PyList_Check($input) ? 1 : 0;
}
%typemap(freearg) const unsigned long* pixels {
    if ($1) {
        free($1);
    }
}
...
%extend GFitsImageULong {
    unsigned long __getitem__(int GTuple1D4D[]) {
    ...
    void __setitem__(int GTuple1D4D[], unsigned long value) {
        if (GTuple1D4D[0] == 1) {
    ...
    PyObject* pixels(void) {
        ...
        for (int i = 0; i < self->npix(); ++i) {
            PyList_SetItem(list, i, PyInt_FromLong((long)(static_cast<unsigned long*>(self->pixels())[i])));
        }
        return list;
    }
